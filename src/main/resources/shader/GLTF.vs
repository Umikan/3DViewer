#version 460

layout(location = 0) in vec3 vertexPosition_modelspace;
layout(location = 1) in vec3 vertexNormal_modelspace;
layout(location = 2) in vec2 texcoord;
layout(location = 3) in vec4 joint;
layout(location = 4) in vec4 weight;
layout(location = 5) uniform int jointSize;
layout(location = 200) uniform mat4 jointMatrix[1024];
layout(location = 10) in vec3 vertexPosition_morph[8];
layout(location = 20) in vec3 vertexNormal_morph[8];
layout(location = 100) uniform int morphSize;
layout(location = 101) uniform float weights[8];
uniform mat4 MVP; 
uniform mat4 M; 
uniform mat4 V; 
uniform vec3 LightPosition_worldspace;

out vec3 EyeDirection_cameraspace;
out vec3 LightDirection_cameraspace;
out vec3 Normal_cameraspace;
out vec2 f_texcoord;

void main(){
    f_texcoord = texcoord;

    vec4 position = vec4(vertexPosition_modelspace, 1.0);
    vec3 normal = vertexNormal_modelspace;
    
    //TODO: which is first, morphing or skinning 
    //morphing
    for (int i = 0; i < 8; i++){
        if (i >= morphSize) break;
        position += weights[i] * vec4(vertexPosition_morph[i].xyz, 0.0);    
        normal += weights[i] * vertexNormal_morph[i];   
    }

    //skinning
    if (jointSize != 0){
        mat4 skinMatrix = weight.x * jointMatrix[int(joint.x)]
                    + weight.y * jointMatrix[int(joint.y)]
                    + weight.z * jointMatrix[int(joint.z)]
                    + weight.w * jointMatrix[int(joint.w)];
        position = skinMatrix * position;
        normal = (skinMatrix * vec4(normal, 0.0)).xyz;
    }

    gl_Position =  MVP * vec4(position.xyz, 1.0);

    //vertex to camera
    vec3 vertexPosition_cameraspace = ( V * M * position).xyz;
    EyeDirection_cameraspace = vec3(0,0,0) - vertexPosition_cameraspace;

    //light direction from vertex
    vec3 LightPosition_cameraspace = ( V * vec4(LightPosition_worldspace,1)).xyz;
    LightDirection_cameraspace = LightPosition_cameraspace + EyeDirection_cameraspace;

    Normal_cameraspace = ( V * inverse(transpose(M)) * vec4(normal,0)).xyz; 
    // Scaling the model, M must be inverse(transpose(M)).
}